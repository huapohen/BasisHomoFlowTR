...
def inference(model, params):
    ...
    with tqdm(total=len(dataloaders['test'])) as t:
        for i, inputs in enumerate(dataloaders['test']):
            ...
            loss = compute_losses(output, inputs, params)
            imgs = [
                output['img_ga_m'],
                output['img_a_m'],
                output['img_a_pred'],
            ...
            imgs = [to_cv2_format(im) for im in imgs]
            ...
            cv2.putText(imgs[3], 'inp', (280, 400), *draw_info)
            ...
            rgbs = [to_rgb(i) for i in imgs]
            i12, i01, i02 = [rgbs[1], rgbs[2]], [rgbs[0], rgbs[1]], [rgbs[0], rgbs[2]]
            ...
            cv2.imwrite(f'{svd}/{i}_avm_inp.jpg', imgs[3])
            cv2.imwrite(f'{svd}/{i}_avm_pred.jpg', imgs[2])
            avm_gt = cv2.imread(avm_gt_path)
            cv2.putText(avm_gt, 'gt', (280, 450), *draw_info)
            cv2.imwrite(f'{svd}/{i}_avm_gt.jpg', avm_gt)
            # shutil.copy2(avm_gt_path, f'{svd}/{i}_avm_gt.jpg')
            to_save_mask(f'{svd}/{i}_mask.jpg', output['ones_mask_w_avm'])

            t.set_description(desc=f"{loss['total']:.4f}")
            t.update()
