    ...
    def forward(self, x):
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            if l == 0:
                ...
                scale = h_x / float(h_patch)
                offset_1 = weight_f.reshape(bs, 4, 2)

                points_2_pred = self.corners + offset_1
                corners = self.corners.expand_as(points_2_pred)
                homo_12 = basenet.dlt_homo(corners*scale, points_2_pred*scale)

                x2_warp = basenet.warp_image_from_H(homo_12, x2, *bhw)
...
class Class_Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        q = self.q(x[:, :1]).reshape(B, 1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3) # fixed position (inspired by Vit)
        ...
        attn = attn.softmax(dim=-1)  # bs, 24, 3072    bs 24 48 64
